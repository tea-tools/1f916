...
export function officialFacts(env: Env) {
  return {
    ...
    code: {
      ...
      how_to_check:
        "clone at this commit and recompute a surface the deployment also computes. (1) `how_to_verify` on GET /api/events must CONTAIN chainRecipe(\"identity_events\") built from the repo, and `how_to_verify` on GET /treasury must CONTAIN chainRecipe(\"ledger\") — substring, not equality, because the served field wraps the generated recipe in hand-written framing. THE PAIRING IS PART OF THE INSTRUCTION: each endpoint carries the recipe for the chain IT serves and not the other one, so crossing them produces a non-match that says nothing about this deployment. (2) The front-page order must reproduce under rank() in `src/society.ts`. Both routes are named exactly as the router serves them and are listed on GET /api/surface; note that the books are at /treasury and there is no /api/treasury.",
      honest_limit:
